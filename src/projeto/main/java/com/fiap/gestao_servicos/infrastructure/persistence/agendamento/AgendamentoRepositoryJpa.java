...
public interface AgendamentoRepositoryJpa extends JpaRepository<AgendamentoEntity, Long> {
	...
	boolean existsByIdAndProfissionalIdAndEstabelecimentoId(Long id, Long profissionalId, Long estabelecimentoId);

	List<AgendamentoEntity> findByStatusAndDataHoraInicioBetween(AgendamentoStatus status,
																 LocalDateTime inicio,
																 LocalDateTime fim);

	Page<AgendamentoEntity> findByEstabelecimentoId(Long estabelecimentoId, Pageable pageable);
	...
	@Query("""
		...
		""")
	boolean existsConflitoHorarioProfissional(@Param("profissionalId") Long profissionalId,
